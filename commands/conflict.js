...
export async function conflict(message, args, gameData) {
  ...
  if (game.players[playerNumericId].hopeDieActive) {
  ...
  if (ones > 0) {
    ...
    const burnTraitConfirmation = await getDMResponse(player.user, `You rolled one or more 1s. Do you want to burn your ${topTrait} to reroll all 1s?`, CONSENT_TIMEOUT, m => m.author.id === playerId);
  ...
  if (game.players[playerNumericId].momentBurned &&
    ...
    const channelBrinkFilter = (m) => m.author.id === playerId && (m.content.toLowerCase() === 'y' || m.content.toLowerCase() === 'n');
    const channelBrinkCollector = message.channel.createMessageCollector({ filter: channelBrinkFilter, time: BRINK_TIMEOUT, max: 1 });
    channelBrinkCollector.on('collect', async (m) => {
      await m.delete();
      const response = m.content.toLowerCase();
      if (response === 'y') {
        await respondViaDM(message, 'You responded "Yes" in the channel. You embraced your Brink!', 'brink');
        await brinkMessage.edit({ content: 'You embraced your Brink!', embeds: [], components: [] });
        message.channel.send(`<@${playerId}> embraced their Brink!`);
        game.players[playerNumericId].brinkUsedThisRoll = true;
      } else {
        await respondViaDM(message, 'You responded "No" in the channel. You chose not to embrace your Brink, for now.', 'brink');
        await brinkMessage.edit({ content: 'You chose not to embrace your Brink, for now.', embeds: [], components: [] });
        message.channel.send(`<@${playerId}> chose not to embrace their Brink. The scene will end.`);
      }
      brinkCollector.stop();
      channelBrinkCollector.stop();
    });
    ...
    channelBrinkCollector.on('end', async (collected, reason) => {
      brinkCollector.stop();
    });
  }
  let gmRolls = [];
  ...
  if (totalPlayerSixes === 0) {
    ...
    try {
      ...
      const channelSacrificeFilter = (m) => m.author.id === playerId && (m.content.toLowerCase() === 'y' || m.content.toLowerCase() === 'n');
      const channelSacrificeCollector = message.channel.createMessageCollector({ filter: channelSacrificeFilter, time: SACRIFICE_TIMEOUT, max: 1 });
      channelSacrificeCollector.on('collect', async (m) => {
        await m.delete();
        const response = m.content.toLowerCase();
        if (response === 'y') {
          await respondViaDM(message, 'You responded "Yes" in the channel. You have chosen to sacrifice your character for narration rights!', 'sacrifice');
          await sacrificeMessage.edit({ content: `You have chosen to sacrifice your character for narration rights!`, embeds: [], components: [] });
          ...
          const reason = await getDMResponse(player.user, `What was the cause of your character's death? (Optional)`, CONSENT_TIMEOUT, m => m.author.id === playerId);
          await died(message, [`<@${playerId}>`, reason]);
        } else {
          await respondViaDM(message, 'You responded "No" in the channel. You chose not to sacrifice your character.', 'sacrifice');
          await sacrificeMessage.edit({ content: 'You chose not to sacrifice your character.', embeds: [], components: [] });
        }
        sacrificeCollector.stop();
        channelSacrificeCollector.stop();
      });
      ...
      channelSacrificeCollector.on('end', async (collected, reason) => {
        sacrificeCollector.stop();
      });
    } catch (error) {
